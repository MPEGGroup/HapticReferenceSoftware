...
namespace haptics::tools {
	...
	std::vector<T> firls(int length, vector<T> freq, const vector<T>& amplitude) {
		...
		for (int i = 0; i < freqSize; i += 2) {
			...
			if (Nodd) {
				b0 += (b1 * (Fip1 - Fi)) +
					m_s / 2 * (std::pow(Fip1, 2) - std::pow(Fi, 2)) * wt2;
			}
			std::transform(
				b.begin(), b.end(), k.begin(), b.begin(),
				[m_s, Fi, Fip1, wt2](T b, T k) {
					return b + (m_s / (4 * std::pow(M_PI, 2)) *
						(std::cos(2 * M_PI * Fip1) - std::cos(2 * M_PI * Fi)) /
						(std::pow(k, 2))) *
						wt2;
				});
			std::transform(b.begin(), b.end(), k.begin(), b.begin(),
				[m_s, Fi, Fip1, wt2, b1](T b, T k) {
					return b +
						(Fip1 * (m_s * Fip1 + b1) * sinc<T>(2 * k * Fip1) -
		...
		auto w0 = weight[0];
		vector<T> a(b.size());
		std::transform(b.begin(), b.end(), a.begin(),
	...
	template <typename T>
	void resample(int upFactor, int downFactor, vector<T>& inputSignal,
		vector<T>& outputSignal) {
		const int n = 10;
		const T bta = 5.0;
